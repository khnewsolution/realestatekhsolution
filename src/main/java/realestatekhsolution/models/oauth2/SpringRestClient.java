...
public class SpringRestClient {
    ...
    public static final String REST_SERVICE_URI = "http://localhost:8080/auth";
    
    public static final String AUTH_SERVER_URI = "http://localhost:8080/auth/oauth/token";
    ...
    private static void getUser(AuthTokenInfo tokenInfo){
        ...
        RestTemplate restTemplate = new RestTemplate();
        HttpEntity<String> request = new HttpEntity<String>(getHeaders());
        ResponseEntity<SecUser> response = restTemplate.exchange(REST_SERVICE_URI+"/user/1"+QPM_ACCESS_TOKEN+tokenInfo.getAccess_token(),
        		HttpMethod.GET, request, SecUser.class);
        SecUser user = response.getBody();
    ...
    private static void updateUser(AuthTokenInfo tokenInfo) {
        ...
        user.setActive(1);
        user.setEmail("dara@gmail.com");
        user.setLastName("daraLU");
        user.setName("daraFU");
        user.setPassword("1");

        HttpEntity<Object> request = new HttpEntity<Object>(user, getHeaders());
        ResponseEntity<SecUser> response = restTemplate.exchange(REST_SERVICE_URI+"/user/1"+QPM_ACCESS_TOKEN+tokenInfo.getAccess_token(),
        		HttpMethod.PUT, request, SecUser.class);
    ...
    private static void deleteUser(AuthTokenInfo tokenInfo) {
        ...
        restTemplate.exchange(REST_SERVICE_URI+"/user/3"+QPM_ACCESS_TOKEN+tokenInfo.getAccess_token(),
        		HttpMethod.DELETE, request, SecUser.class);
    }
    ...
    private static List<SecUser> getAllUsers(AuthTokenInfo tokenInfo){
        Assert.notNull(tokenInfo, "Authenticate first please......");
